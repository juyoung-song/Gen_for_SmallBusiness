...
### [feature/won/custom_img_prompt] 이미지 프롬프트 커스텀 힌트 입력
- 이미지 생성 시 분위기·소품·색감 등을 자유롭게 직접 입력할 수 있는 힌트 입력란 추가 (3단계 옵션 섹션)
- 입력된 힌트는 프롬프트 앞쪽에 우선 배치되어 이미지 생성에 직접 반영
- GPT 번역 시스템 프롬프트에 힌트 보존 지시어 추가 (임의 삭제·재해석 방지)
- `schemas/image_schema.py`: `image_prompt_hint` 필드 추가
- `utils/prompt_builder.py`: `extra_hint` 파라미터 추가
- `services/image_service.py`: 힌트 전달 및 GPT 지시어 강화
